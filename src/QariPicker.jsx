...
export default function QariPicker({ value, onChange, label = 'Pilih qari' }) {
  const selectedId = qariFor(value).id
  return <div className="qari-picker" role="radiogroup" aria-label={label}>
    {QARIS.map((qari) => {
      ...
      return <button type="button" role="radio" aria-checked={selected} key={qari.id} onClick={() => onChange(qari.id)} className={`qari-choice ${selected ? 'qari-choice-active' : ''}`}>
        ...
        {qari.id === 'ayman-sowaid' && <span className="qari-default-badge">Bawaan</span>}
      </button>
    })}
  </div>
}
