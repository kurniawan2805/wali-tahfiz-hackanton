...
export default function QariPicker({ value, onChange, label }) {
  ...
  return <div className="qari-picker" role="group" aria-label={label || t('qari.label')}>
    {QARIS.map((qari) => {
      const selected = qari.id === selectedId
      return <button type="button" aria-pressed={selected} key={qari.id} onClick={() => onChange(qari.id)} className={`qari-choice ${selected ? 'qari-choice-active' : ''}`}>
