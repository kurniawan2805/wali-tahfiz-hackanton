...
export default function SettingsPage({ family, save, back, onExport, onImport, onReset, qariId = DEFAULT_QARI_ID, saveQari, locale, saveLocale, theme = 'light', setTheme, ui }) {
  ...
  const handleImport = async (event) => {
    ...
    try {
      ...
      back()
    } catch (error) { setTransferStatus(error instanceof Error ? error.message : 'File cadangan could not diimpor.') }
    finally { setIsTransferring(false) }
  }
  ...
  return <main className="app-page pb-32"><div className="page-shell page-shell-settings">
    ...
    <SettingsAccordion id="welcome" openSection={openSection} onToggle={toggleSection} content={<button type="button" onClick={() => window.localStorage.removeItem('hasSeenAgentWelcome')} className="secondary-button mt-5"><Settings size={18}/>Show again when the home page opens</button>}><div className="settings-section-heading"><p className="step-label">TEMAN HAFALAN</p><span>Welcome popup</span><p>Reset the Memorisation Companion popup so it appears again when opening the home page.</p></div></SettingsAccordion>
    ...
    <SettingsAccordion id="danger" openSection={openSection} onToggle={toggleSection} className="settings-section settings-section-danger" content={<button type="button" onClick={() => setShowResetConfirmation(true)} className="secondary-button mt-5 !border-[#eab59a] !bg-white !text-terracotta dark:!bg-[#16211E]"><Trash2 size={18}/>{t('settings.reset')}</button>}><div className="settings-section-heading"><p className="step-label"><AlertTriangle size={13}/> {t('settings.dangerLabel')}</p><span>{t('settings.dangerTitle')}</span><p>{t('settings.dangerBody')}</p></div></SettingsAccordion>
  ...
  </div>{pendingDeletion && <div className="sheet-backdrop" role="presentation"><section className="sheet sm:max-w-md" role="dialog" aria-modal="true" aria-labelledby="delete-child-title"><p className="step-label bg-peach text-terracotta">DELETE CONFIRMATION</p><h2 id="delete-child-title" className="font-display mt-3 text-2xl text-forest">Delete {pendingDeletion.name || 'child profile'}?</h2><p className="mt-2 text-sm leading-relaxed text-slate-600">Today’s targets and saved memorisation for {pendingDeletion.name || 'this child'} will also be deleted when settings are saved. This action cannot be undone.</p><div className="mt-6 flex gap-3"><button type="button" onClick={() => setPendingDeletion(null)} className="secondary-button">Cancel</button><button type="button" onClick={confirmDelete} className="primary-button !bg-terracotta">Delete child</button></div></section></div>}{showResetConfirmation && <div className="sheet-backdrop" role="presentation"><section className="sheet sm:max-w-md" role="dialog" aria-modal="true" aria-labelledby="reset-data-title"><p className="step-label bg-peach text-terracotta"><AlertTriangle size={13}/> RESET CONFIRMATION</p><h2 id="reset-data-title" className="font-display mt-3 text-2xl text-terracotta">Delete all data?</h2><p className="mt-2 text-sm leading-relaxed text-slate-600">Data cannot be recovered unless you have exported a backup. You will return to the home page.</p><div className="mt-6 flex gap-3"><button type="button" onClick={() => setShowResetConfirmation(false)} className="secondary-button">Cancel</button><button type="button" onClick={handleReset} className="primary-button !bg-terracotta">Yes, delete everything</button></div></section></div>}</main>
}
