...
import { audioUrlForAyah, audioUrlForSurah, DEFAULT_QARI_ID, QARIS, qariFor } from './quranAudio'
...
export default function QuranRangePage({ back, qariId = DEFAULT_QARI_ID, onQariChange }) {
  ...
  const isSurahPlayback = qari.mode === 'surah'
  ...
  const audioUrl = selected && (isBismillah || activeAyah) ? (isSurahPlayback ? audioUrlForSurah(selectedQariId, selected.id) : isBismillah ? audioUrlForAyah(selectedQariId, 1) : audioUrlForAyah(selectedQariId, activeVerse?.globalNumber)) : undefined
  ...
  const startRange = () => {
    resetProgress()
    if (isSurahPlayback) playFromAyah(1)
    else if (selectedId !== '1' && rangeStart === 1) playBismillah()
  ...
  const handleEnded = () => {
    if (isSurahPlayback) { resetProgress(); setIsPlaying(false); return }
    if (isBismillah) {
      setIsBismillah(false)
